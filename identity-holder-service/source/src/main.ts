...
import { initializeRootHolderAgent } from './agent/agent-holder'
...
async function bootstrap(): Promise<void> {
  ...
  const corsOrigin =
    cfg.walletOrigin === '*'
      ? true
      : cfg.walletOrigin.split(',').map((o) => o.trim()).filter(Boolean)
  app.enableCors({
    origin: corsOrigin,
  ...
  await initializeRootHolderAgent(
    expressApp,
    wss,
    recordStorage,
    keyManagementService,
  ...
  logger.log(`Listening on ${port} (HTTP API + DIDComm WS)`, 'Holder')
}
...
bootstrap().catch((err) => {
  ...
  const nested =
    cause && typeof cause === 'object' && cause !== null && 'cause' in cause
      ...
      : undefined
  logger.error('Bootstrap failed', err, 'Holder')
  if (cause) {
    logger.error(
      'Bootstrap cause',
