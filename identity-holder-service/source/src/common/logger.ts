 ...
 * (incluidos los internos del framework) se emita como un objeto JSON
 * de una sola línea a stdout/stderr.
 * El nivel de log es configurable mediante la variable de entorno `LOG_LEVEL`.
 */
@Injectable()
export class JsonLoggerService implements LoggerService {
  private readonly service = 'identity-holder'
   ...
   *
   * Sigue la convención de NestJS donde el último parámetro string se trata
   * como el contexto de logging (nombre de clase/módulo) y todo lo intermedio
   * se recopila en un arreglo `details`.
   *
   * Los logs ERROR van a stderr; todos los demás niveles van a stdout para que
   * los entornos de contenedores (Docker, K8s) y los recolectores de logs
   * puedan clasificarlos automáticamente.
   */
  ...
  /** Extrae nombre, mensaje y stack de instancias de Error; deja pasar otros valores sin cambios. */
  private formatMessage(message: unknown): unknown {
    if (message instanceof Error) {
      ...
      if (message instanceof AggregateError) {
        errorData.errors = message.errors.map((e) => this.formatMessage(e))
      }
      return errorData
    }
    return message
