...
Consume `@identity/core` para inicializar un agente Credo-TS con capacidades de emisión DIDComm y OID4VCI (SD-JWT), además de exponer endpoints de gestión de issuers, consulta de records Credo, metadata OID4VCI y StatusList (revocación).

> Este servicio es un entorno de prueba/integración para validar `@identity/core`. No forma parte del stack productivo (Gateway, Auth, Web, Index, Resolver).
...
`identity-issuer-service` actúa como emisor (`did:web`) dentro del ecosistema QuarkID 2.0. Ofrece credenciales al holder por dos vías — DIDComm (OOB) y OID4VCI (SD-JWT pre-authorized) —, registra y resuelve DIDs contra el VDR Service, persiste StatusLists y revocaciones en Postgres, y publica eventos a RabbitMQ (`quarkid.index` y `quarkid.audit`).

### Diagrama 1 — Ecosistema (resaltando issuer)

```mermaid
graph LR
    holder["identity-holder-service<br/>(Holder · did:key)"]
    issuer["identity-issuer-service<br/>(Issuer · did:web)"]:::focus
...
│   ├── revocation.service.ts        # wrapper de @identity/core
...
| `BASE_URL`     | `http://identity-issuer-service:9001` | URL base pública. Se usa para construir `didcommEndpoint`, `invitationUrlPrefix` y el prefijo OID4VCI. |
...
- `POST /issuers/:walletId/revocation/status-list/:vct/revoke` mapea `CredentialAlreadyRevokedError` (de `@identity/core`) a `409 Conflict` con `{ statusCode, message }`.
