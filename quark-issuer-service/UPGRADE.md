...
1. **TypeORM eliminado del issuer.** El módulo `RevocationIssuerModule` ya no usa TypeORM. Las tablas `status_lists` y `status_list_revocations` ahora las crea el adapter `PostgresStatusListStorage` de `@quarkid/identity-core` con DDL idempotente al primer arranque.
...
3. **Cambios en DI (consumidores de `@quarkid/identity-core`)**:
