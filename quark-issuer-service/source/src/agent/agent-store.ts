...
import { withTenant, type Agent } from '@quarkid/identity-core'
...
/** Pares `walletId` → `tenantId` registrados en este proceso (incluye tenants cargados al bootstrap). */
...
/**
 * Registra un tenant recién creado en el mapa en memoria (`walletId` → `tenantId` de Credo).
 * Necesario tras `POST /issuers` para que `withWallet` funcione sin reiniciar el proceso.
 */
