...
const navLinkClass =
  'inline-flex items-center gap-2 hover:text-[var(--kuatia-text)]'
  ...
  return (
    ...
    >
      ...
      <nav className="order-3 hidden w-full items-center justify-center gap-6 text-base text-[var(--kuatia-muted)] sm:order-none sm:flex sm:w-auto md:gap-8 lg:gap-10">
        ...
        <Link href={`${prefix}#precios`} className={navLinkClass}>
          <IconPlan size={18} />
          Precios
        </Link>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        <ThemeToggle />
        {loading ? (
          <span className="h-10 w-28 animate-pulse rounded-xl bg-[var(--kuatia-hover)]" aria-hidden />
        ) : account ? (
