# Quark identity monorepo (snapshot)
...
| `quark-issuer-service` | Emisión (OID4VCI, DIDComm) — `:9001` |
| `quark-verifier-service` | Verificación (OID4VP, DIDComm) — `:9002` |
| `quark-holder-service` | Custodia / presentación — `:9005` |
| `quark-wallet` | App Flutter |
...
cp quark-issuer-service/source/.env.example quark-issuer-service/source/.env
cp quark-verifier-service/source/.env.example quark-verifier-service/source/.env
cp quark-holder-service/source/.env.example quark-holder-service/source/.env
...
- RabbitMQ UI: http://localhost:15672 (`quarkid` / `quarkid`)  
- pgAdmin: http://localhost:5050  

Postman: importar `postman/` y el environment `Quark-Local-Docker`.
