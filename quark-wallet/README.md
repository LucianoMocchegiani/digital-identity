...
Construido sobre [`identity_core_dart`](../packages/identity-core-dart) — el SDK SSI nativo de QuarkID para Dart/Flutter.
...
│   ├── quark_shared.dart        Barrel: un solo import para extensions, theme, layout, utils y widgets de flujo
...
Ahí se reexportan extensiones de navegación, `kAuthOnboardingScreenPadding`, estilos de botón y títulos (`quarkPageTitle`, `quarkHeroTitle`), snackbars, vistas de flujo (carga, error, éxito), `FlowStepAppBar`, `FlowActionRow`, `NetworkLogoOrPlaceholder`, `AppSixDigitPin`, etc. Algunos widgets internos de `shared` pueden seguir importando un archivo concreto (p. ej. tema PIN) para evitar dependencias circulares.
...
| UI transversal | `lib/shared` + barrel `quark_shared.dart` |
