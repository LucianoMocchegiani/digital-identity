...
import { BBS_PROOF_TYPE } from '../../credential/bbs/constants'
...
/**
 * (Local `findTenantIdForRecord` removido: ahora se usa el exportado desde
 * `./shared.listener` para evitar duplicación entre issuer/holder/verifier.)
 */
        ...
        switch (record.state) {
          case DidCommCredentialState.OfferReceived:
            log.log('Holder: Offer received, accepting...')
            {
              ...
              if (!tenantId) {
                log.warn(`[Holder] No tenant found for credential record ${record.id}, skipping acceptOffer`)
                break
              }
              ...
              await api.withTenantAgent({ tenantId }, async (tenantAgent) => {
                const a = tenantAgent as unknown as {
                  didcomm: {
                    credentials: {
                      acceptOffer(opts: unknown): Promise<unknown>
                      getFormatData(id: string): Promise<unknown>
                    }
                  }
                }
                // Request/offer gana: si el offer trae proofType, usarlo; si no, default BBS.
                const formatData = await a.didcomm.credentials.getFormatData(record.id)
                const offerProofType = (
                  formatData as {
                    offer?: { jsonld?: { options?: { proofType?: string } } }
                  }
                ).offer?.jsonld?.options?.proofType
                await a.didcomm.credentials.acceptOffer({
                  credentialExchangeRecordId: record.id,
                  credentialFormats: {
                    jsonld: {
                      options: {
                        proofType: offerProofType ?? BBS_PROOF_TYPE,
              ...
              log.log(`[Holder] Offer accepted within tenant ${tenantId.slice(0, 8)}…`)
            }
            break
          case DidCommCredentialState.CredentialReceived:
            log.log('Holder: Credential received, accepting...')
            {
              const tenantId = await findTenantIdForRecord(agent, record.id, 'credentials')
              if (!tenantId) {
                log.warn(`[Holder] No tenant found for credential record ${record.id}, skipping acceptCredential`)
                break
              }
              const api = agent.dependencyManager.resolve(TenantsApi)
              await api.withTenantAgent({ tenantId }, async (tenantAgent) => {
                const a = tenantAgent as unknown as {
                  didcomm: { credentials: { acceptCredential(opts: unknown): Promise<unknown> } }
                }
                await a.didcomm.credentials.acceptCredential({
                  credentialExchangeRecordId: record.id,
                })
              })
            }
    ...
    async (ev: { payload?: { proofRecord?: DidCommProofExchangeRecord; proofExchangeRecord?: DidCommProofExchangeRecord } }) => {
      ...
      try {
        if (record.state === DidCommProofState.RequestReceived) {
          ...
          if (!tenantId) {
            log.warn(`[Holder] No tenant found for proof record ${record.id}, skipping`)
            return
          }
          const api = agent.dependencyManager.resolve(TenantsApi)
          await api.withTenantAgent({ tenantId }, async (tenantAgent) => {
            const a = tenantAgent as unknown as {
              didcomm: {
                proofs: {
                  selectCredentialsForRequest(opts: unknown): Promise<{ proofFormats: unknown }>
            ...
            if (pex) {
              await expandPexSelection(tenantAgent as unknown as Agent, record.id, pex, log)
            }
